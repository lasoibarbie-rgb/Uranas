import os, urllib.parse, base64, requests, re, csv, io, secrets
...
os.makedirs(os.path.join(UPLOAD_DIR,"bins"), exist_ok=True)
os.makedirs(os.path.join(UPLOAD_DIR,"bnb"), exist_ok=True)
...
def ensure_schema():
    ...
    if "client" in inspector.get_table_names():
        cols={x["name"] for x in inspector.get_columns("client")}
        if "location" not in cols:
            with db.engine.begin() as conn:
                conn.exec_driver_sql("ALTER TABLE client ADD COLUMN location VARCHAR(200)")
    db.create_all()

def save_image(upload, category):
    if category not in {"bin","bnb"} or not upload or not upload.filename:
        raise ValueError("Choose a Bin or BNB image.")
    filename=secure_filename(upload.filename)
    ext=filename.rsplit(".",1)[-1].lower() if "." in filename else ""
    if ext not in ALLOWED_IMAGE_EXTENSIONS:
        raise ValueError("Allowed images: JPG, JPEG, PNG, WEBP or GIF.")
    final=secrets.token_hex(8)+"_"+filename
    folder=os.path.join(UPLOAD_DIR,category)
    os.makedirs(folder,exist_ok=True)
    upload.save(os.path.join(folder,final))
    return final

def normalize_import_row(raw):
    return {str(k).strip().lower():("" if v is None else str(v).strip()) for k,v in dict(raw).items()}
...
@app.route("/")
def home():
    bin_media=WebsiteMedia.query.filter_by(category="bin").order_by(WebsiteMedia.created_at.desc()).all()
    bnb_media=WebsiteMedia.query.filter_by(category="bnb").order_by(WebsiteMedia.created_at.desc()).all()
    return render_template("home.html",bin_media=bin_media,bnb_media=bnb_media)
...
def debts():
    ...
    invoices=Invoice.query.order_by(Invoice.due_date.asc().nullslast(), Invoice.created_at.desc()).all()
    if status in ("overdue","due","paid"):
        invoices=[i for i in invoices if i.status.lower()==status]
    total=sum((Decimal(str(i.balance)) for i in invoices),Decimal("0"))
    return render_template("debts.html", invoices=invoices, status=status, total=total)
...
def import_data():
    ...
    if request.method=="POST":
        try:
            ...
            from data_import_service import name_key, norm
            payments_file=request.files.get("payments")
            contacts_file=request.files.get("contacts")
            packaged_payments=os.path.join(os.path.dirname(__file__),"data_import","JAN-AUGUST PAYMENTS UPDATE-1.xlsx")
            packaged_contacts=os.path.join(os.path.dirname(__file__),"data_import","Contact list updated 2.docx")
            pay_path=packaged_payments
            if payments_file and payments_file.filename:
                pay_path="/tmp/uranas-payments-upload.xlsx"; payments_file.save(pay_path)
            contact_path=packaged_contacts
            if contacts_file and contacts_file.filename:
                contact_path="/tmp/uranas-contacts-upload.docx"; contacts_file.save(contact_path)

            wb=load_workbook(pay_path,data_only=True)
            ws=wb.active
            months=[]
            for col in range(5,17):
                v=ws.cell(2,col).value
                if v: months.append((col,str(v).strip().title()))
            imported_clients=imported_invoices=imported_payments=updated_contacts=0
            existing={name_key(c.name):c for c in Client.query.all()}
            # Import payment workbook rows. Section headings are skipped.
            for r in range(4,ws.max_row+1):
                raw=ws.cell(r,1).value
                name=str(raw).strip() if raw is not None else ""
                if not name or name.upper().endswith("CLIENTS") or name.upper() in {"NAIROBI","KAWANGWARE"}:
                    continue
                try: bins=int(ws.cell(r,2).value or 0)
                except: bins=0
                try: unit=amount(ws.cell(r,3).value or 0)
                except: unit=Decimal("0")
                try: total=amount(ws.cell(r,4).value or 0)
                except: total=Decimal(str(bins))*unit
                key=name_key(name)
                c=existing.get(key)
                if not c:
                    c=Client(name=name,service="Sanitary bin service")
                    db.session.add(c); db.session.flush(); existing[key]=c; imported_clients+=1
                # Add/update one recurring item without duplicating it.
                item=next((x for x in c.items if norm(x.description)==norm("Sanitary bin service")),None)
                if bins>0:
                    if not item:
                        item=ServiceItem(client_id=c.id,description="Sanitary bin service",quantity=bins,unit_price=unit,active=True)
                        db.session.add(item)
                    else:
                        item.quantity=bins; item.unit_price=unit; item.active=True
                for col,month in months:
                    status=str(ws.cell(r,col).value or "").strip().upper()
                    if status not in {"PAID","PENDING","CLOSED"}: continue
                    period=f"2026-{dict((m,idx+1) for idx,(cidx,m) in enumerate(months)).get(month,1):02d}"
                    if Invoice.query.filter_by(client_id=c.id,period=period).first(): continue
                    due=date(2026,dict((m,idx+1) for idx,(cidx,m) in enumerate(months)).get(month,1),30)
                    inv=Invoice(client_id=c.id,invoice_no=make_invoice_no(),description=f"Monthly sanitary bin service - {month} 2026",total=total,paid=(total if status in {"PAID","CLOSED"} else Decimal("0")),due_date=due,period=period)
                    db.session.add(inv); db.session.flush(); imported_invoices+=1
                    if status in {"PAID","CLOSED"}:
                        db.session.add(Payment(invoice_id=inv.id,amount=total,method="Historical spreadsheet",reference=f"Imported {month} 2026",paid_on=due)); imported_payments+=1

            # Parse the DOCX text with a fallback that ignores damaged embedded media.
            try:
                from docx import Document
                doc=Document(contact_path)
                lines=[p.text.strip() for p in doc.paragraphs if p.text.strip()]
            except Exception:
                import zipfile, xml.etree.ElementTree as ET
                with zipfile.ZipFile(contact_path) as z: xml=z.read("word/document.xml")
                root=ET.fromstring(xml); ns={"w":"http://schemas.openxmlformats.org/wordprocessingml/2006/main"}
                lines=[''.join(x.text or '' for x in para.findall('.//w:t',ns)).strip() for para in root.findall('.//w:p',ns)]
                lines=[x for x in lines if x]
            phones=re.compile(r'^(?:\+?254|0)(?:1|7)\d{8}$')
            i=0
            while i<len(lines):
                line=lines[i].strip(); key=name_key(line)
                if key in existing and not phones.match(re.sub(r'\D','',line)):
                    c=existing[key]
                    j=i+1
                    found=None
                    while j<min(i+5,len(lines)):
                        digits=re.sub(r'\D','',lines[j])
                        if phones.match(digits): found=normalize_phone(digits); break
                        j+=1
                    if found and not c.phone:
                        c.phone=found; updated_contacts+=1
                i+=1
            db.session.commit()
            message=f"Imported/updated {imported_clients} clients, {imported_invoices} historical monthly invoices, {imported_payments} payment records, and {updated_contacts} contact numbers."
        except Exception as e:
            db.session.rollback(); flash(f"Import failed: {e}","error")
    return render_template("import.html", message=message)


@app.route("/admin/import/clients", methods=["GET","POST"])
@auth
def import_clients():
    imported=skipped=0
    if request.method=="POST":
        ...
        if not f or not f.filename:
            flash("Choose a CSV, XLS or XLSX client file.","error")
            return redirect(url_for("import_clients"))
        ext=f.filename.rsplit(".",1)[-1].lower() if "." in f.filename else ""
        if ext not in {"csv","xlsx","xls"}:
            flash("Use CSV, XLS or XLSX.","error")
            return redirect(url_for("import_clients"))
        try:
            if ext=="csv":
                ...
                if ext=="xlsx":
                    ...
                    ws=wb.active; values=list(ws.values)
                    if not values: raise ValueError("The file is empty.")
            ...
            existing={re.sub(r"[^a-z0-9]","",c.name.lower()):c for c in Client.query.all()}
            for raw in rows:
                r=normalize_import_row(raw)
                name=r.get("name") or r.get("client name") or r.get("client_name") or r.get("customer name")
                if not name:
                    skipped+=1; continue
                key=re.sub(r"[^a-z0-9]","",name.lower())
                c=existing.get(key)
                if not c:
                    c=Client(name=name); db.session.add(c); db.session.flush(); existing[key]=c; imported+=1
                mappings={
                    "phone":["phone","phone number","client phone","mobile","mobile number"],
                    "email":["email","email address"],
                    "location":["location","address","town","city"],
                    "service":["service","services","service type"],
                    "notes":["notes","remarks","comment"]
                }
                for attr,keys in mappings.items():
                    value=next((r[k] for k in keys if r.get(k)),None)
                    if value: setattr(c,attr,value)
            db.session.commit()
            flash(f"Client import complete: {imported} new clients added; {skipped} rows skipped. Matching clients were updated.","success")
        except Exception as e:
            db.session.rollback(); flash(f"Client import failed: {e}","error")
    return render_template("import_clients.html")

@app.get("/admin/import/clients/template")
@auth
def client_import_template():
    data="Name,Phone,Email,Location,Service,Notes\nExample Client,0712345678,client@example.com,Nairobi,Sanitary Bin,Monthly service\n"
    return send_file(io.BytesIO(data.encode()),mimetype="text/csv",as_attachment=True,download_name="uranas_client_import_template.csv")
...
def media_manager():
    if request.method=="POST":
        try:
            ...
            filename=save_image(request.files.get("image"),category)
            m=WebsiteMedia(category=category,title=request.form.get("title","").strip(),
                           description=request.form.get("description","").strip(),filename=filename)
            db.session.add(m); db.session.commit()
            flash("Image published. It is now visible on the public website.","success")
        except Exception as e:
            db.session.rollback(); flash(str(e),"error")
        return redirect(url_for("media_manager"))
    media=WebsiteMedia.query.order_by(WebsiteMedia.created_at.desc()).all()
    return render_template("media.html",media=media)

@app.post("/admin/media/<int:media_id>/delete")
@auth
def delete_media(media_id):
    m=db.get_or_404(WebsiteMedia,media_id)
    path=os.path.join(UPLOAD_DIR,m.category,m.filename)
    try:
        if os.path.exists(path): os.remove(path)
    except OSError: pass
    db.session.delete(m); db.session.commit()
    flash("Website image removed.","success")
    return redirect(url_for("media_manager"))

@app.get("/uploads/<category>/<path:filename>")
def uploaded_media(category,filename):
    if category not in {"bin","bnb"}: return "Not found",404
...
def new_client():
    if request.method=="POST":
        c=Client(name=request.form["name"].strip(),phone=request.form.get("phone"),email=request.form.get("email"),
                 service=request.form.get("service"),location=request.form.get("location"),notes=request.form.get("notes"))
...
def edit_client(client_id):
    ...
    if request.method=="POST":
        ...
        c.service=request.form.get("service"); c.location=request.form.get("location"); c.notes=request.form.get("notes")
...
def service_notification(client_id):
    c=db.get_or_404(Client,client_id)
    if not c.phone:
        flash("This client has no phone number. Add the client's actual number first.","error")
        return redirect(url_for("client",client_id=c.id))
    msg=f"""Hello {c.name},
...
Hygiene today, healthier tomorrow."""
    return redirect("https://wa.me/"+normalize_phone(c.phone)+"?text="+urllib.parse.quote(msg))
